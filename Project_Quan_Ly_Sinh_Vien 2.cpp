...
int MenuCha()
{
	...
	while ( 1 )
	{
		ThongTinChung();
		char key = GetKey();
		if ( key == ENTER || (key == RIGHT && vitri != 4) ) // vitri!= 4 la ngay tai o Thoï¿½t
		{return vitri + 1 ;}
		switch(key)
			{
			case DOWN :
...
//Doc - Ghi File. Xoa - Tim kiem - Dem - Sap Xep, Kiem tra trï¿½ng,...
		...
		Head = p;
			...
			for(ListSV *k = Head; k != NULL; k = k->Next) 
			{
				if(k->Next == NULL)	//Tim ra Node Tail
				{
					k->Next = p;
					k = p;
					Cap_Nhat_MH_SV(Head,k->sv); // Truong hop DSSV da duoc cap nhat diem cï¿½c mon hoc. user nhap them 1 sinh vien moi.
...
ListSV* FindSV(ListLop &dsLOP, char c[])
{
	for(int i = 0; i < dsLOP.n; i++) //Duyet Lop
	{
		...
		{
			if(strcmp(p->sv.MASV, c) == 0)		{return p;} //Tim thay sinh vien => tra ve nï¿½t svien 
		}
	}return NULL;
}
	...
	for(int i = 0; i < dsLOP.n; i++) // Duyet Lop
	{
		for(ListSV *p = dsLOP.l[i].Head ; p != NULL; p = p->Next) //Duyet Sinh Vien
		{
			...
			{	
				...
				f << -k->d.LAN << endl; // Gan gia tri so' lan` la -1 lï¿½ LAN 1, -2 lï¿½ LAN 2
				f << k->d.MAMH << endl;
			}
		}
	}
...
void PhanTuTheMang(ListMonHoc *&p, ListMonHoc *&q)
{
	if(q->left != NULL)
	{
		...
		p->mh = q->mh;				//cap nhat du lieu vï¿½ cï¿½ng trï¿½i sang con tro can xï¿½a
		p = q;						//cap nhat dia chi vï¿½ cï¿½ng trï¿½i sang con tro can xï¿½a
		q = q->right;				//ben phai cua vï¿½ cï¿½ng trai' dï¿½n` lï¿½n
	}
}
...
int KTMaLop(ListLop ds, char c[], int demkhongdoi)
{
	for(int i = 0; i <= demkhongdoi; i++)
	{
		...
		{
			return i;	// neu khong phai ptu dau tien trung thi tra ve 1 sï¿½' > 0
		}
	}
...
void LuuFileSV(ListLop ds)
{
	...
	{
		if(ds.l[i].Head != NULL) //Kiem tra Lop cï¿½ Sinh Vien moi' nap. vao`
		{
			f << ds.l[i].MALOP << endl;
...
MALOP:	
	while(1)
	{
		...
		key = GetKey();				//Bat phim
		if(ds.l[ds.n-1].NienKhoa[0] == '\0' && y > 8) //Kiem tra ï¿½ truoc' co du lieu chua
		{
		...
		if(key == ENTER && vitri != 0)
		{
			...
			ds.l[ds.n].MALOP[vitri] = '\0';		  //Ngat chuï¿½i~
...
TENLOP:	
	...
	{	
		...
		key = GetKey();
		if(ds.l[ds.n].MALOP[0] == '\0')		//Kiem tra ï¿½ phï¿½a truoc cï¿½ du~ lieu chua
		{
		...
		if(key == ENTER && vitri != 0)
		{	
			if(ds.l[ds.n].TENLOP[vitri-1] == ' ') 	{vitri--;}	//KT khi enter vi tri cuoi cung lï¿½ khoang trang thi xoa' khoang trang
...
NIENKHOA: 	
	while(1)
	{
		...
		key = GetKey();
		if(ds.l[ds.n].TENLOP[0] == '\0')	//Kiem tra ï¿½ truoc' co du~ lieu chua
			{
		...
		if(key == ENTER && vitri!=0)
		{
			...
			if(ds.l[1+ds.n].MALOP[0] == '\0' && dem == ds.n)	//Khong cho bien dem tang len khi sua loi~ cac NK phia' trï¿½n
			{
...
NIENKHOA:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Ki tu: (0-9). Toi da: 4         ");TextColor(12);gotoXY(62+vitri,y); //Ghi chï¿½
		key = GetKey(); //Bat phim
		if(ds.l[ds.n].TENLOP[0] == '\0')		//Kiem tra ï¿½ phï¿½a truoc cï¿½ du~ lieu chua
		{
			GhiChu("Ban Chua Nhap Ten Lop!",35,27,74,700);
			gotoXY(30,y);
			goto TENLOP;
...
MALOP:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Ki tu: (A-Z) & (0-9). Toi da: 15");TextColor(12);gotoXY(12+vitri,y); //Ghi chï¿½
		key = GetKey(); //Bat phim
		if((( key >= 'A' && key <= 'Z') || ( key <= '9' && key >= '0') || ( key >='a' && key <= 'z' )) && (vitri < 15))
		{
			ds.l[ds.n].MALOP[vitri] = char(key);
...
TENLOP:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Ki tu: (A-Z) & (0-9). Toi da: 30");	TextColor(12);	gotoXY(30+vitri,y); //Ghi chï¿½
		key = GetKey();
		if(ds.l[ds.n].MALOP[0] == '\0')		//Kiem tra ï¿½ phï¿½a truoc cï¿½ du~ lieu chua
		{
			GhiChu("Ban Chua Nhap Ma Lop!",35,27,74,700);
	...
	while(1)
	{
		...
		if(key == ENTER && vitri!=0)
		{
			...
			if( temp != 0)
			{
				...
				check_change = true;
				for(int i = vitriMang; i < ds.n; i++){	ds.l[i] = ds.l[i+1];	} //dï¿½n` lï¿½n giam? di 1 don vi
				ds.n--;
				return;
			}else{
	...
	{		
		...
		if(key == ESC)
		{
			...
			if(check_change == true)
			{
				...
				if(temp == -1)	{continue;}	//Tra ve -1 => Bat' duoc phim' ESC 1 lï¿½n` nua~
				if(temp == 1)	{GhiChu("Luu Vao File Thanh Cong!",35,27,74,2000);LuuFileLop(ds);LuuFileSV(ds); LuuFileDiem(ds,root);}
				if(temp == 0)	{GhiChu("Luu Vao File Khong Thanh Cong!",35,27,74,2000);}
			}
			system("cls");
...
void XuatLop_NienKhoa(ListLop &ds, ListMonHoc *root)
{
	...
	while(1)
	{
		...
		{
			ds.l[ds.n].NienKhoa[vitri] = '\0';
			if(KTNienKhoa(ds,ds.l[ds.n].NienKhoa) == 1) //Ktra trï¿½ng
			{
...
Xuat:
	...
	{	
		...
		{	
			if(Full_Lop(ds)){continue;}
			gotoXY(18,24);	TextColor(251);	printf("F1"); //Tï¿½ mï¿½u F1
			ThemLop(ds,root,check_change);
		}	
		if(key == F2)
		{
			gotoXY(35,24);	TextColor(251);	printf("F2"); //Tï¿½ mï¿½u F2
			SuaLop(ds,root,check_change);
		}
		if(key == F3)
		{
			gotoXY(50,24);	TextColor(251);	printf("F3"); //Tï¿½ mï¿½u F3
			XoaLop(ds,check_change);
		}
		if(key == ESC)
		{
			if(check_change == true)
			{
				putchar(7);	gotoXY(4,24);  TextColor(251);	printf("ESC"); //Tï¿½ mï¿½u ESC
				int temp = ThongBao_ESC();
				if(temp == -1)	{continue;} //Tra ve -1 => Bat' duoc phim' ESC 1 lï¿½n` nua~
				if(temp == 1)	{GhiChu("Luu Vao File Thanh Cong!",35,27,74,2000);LuuFileLop(ds);LuuFileSV(ds); LuuFileDiem(ds,root);}
				if(temp == 0)	{GhiChu("Luu Vao File Khong Thanh Cong!",35,27,74,2000);}	
...
void NhapSinhVien(ListLop &ds, ListMonHoc *root)
{
	...
	while(1)
	{
		...
		{
			ds.l[ds.n].MALOP[vitri] = '\0';
			temp = KTMaLop(ds, ds.l[ds.n].MALOP, ds.n); //Kiem tra trï¿½ng ma lop
			if( temp != 0)
			{
				if(temp == -1)
				{
...
MASV:
	while(1)
	{
		...
		gotoXY(20,31); printf("                                                              "); TextColor(91);gotoXY(10+vitri,y);
		key = GetKey();
		if(demsv[dem-1].SODT[0] == '\0' && y > 8)		//Kiem tra ï¿½ phï¿½a truoc' cï¿½ du~ lieu chua
		{
		...
		if(key == ENTER && vitri!=0)
		{
			...
			for(int i = 0; i <= demkhongdoi; i++)
			{
				if(i == dem){continue;}	// khi user sua loi~ cac MASV o tren bo? qua duyet bï¿½. dï¿½m. chï¿½nh nï¿½
		...
		if(key == ESC)
		{
			if(check_change == true)
			{
				...
				if(temp == -1)	//-1 la Bat duoc 1 phim ESC nua~
				{
					gotoXY(4,24);  TextColor(252);	printf("ESC"); TextColor(91); continue; //Tï¿½ mï¿½u ESC
				}	
				if(temp == 1) // 1 la YES 
				{
...
HO:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Ki tu: (A-Z) & (0-9). Toi da: 20"); TextColor(12); gotoXY(29+vitri,y);//Ghi chï¿½
		key = GetKey();
		if(sv.MASV[0] == '\0' )
		{
			GhiChu("Ban Chua Nhap MA Sinh Vien!",35,27,74,700);
...
PHAI:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Gioi tinh Nam / Nu. Toi da: 3         "); TextColor(12); gotoXY(63+vitri,y); //Ghi chï¿½
		key = GetKey();
		if(sv.TEN[0] == '\0' )
		{
...
SODT:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Ki tu: (0-9). Toi da: 11        ");	TextColor(12);	gotoXY(68+vitri,y); // Ghi chï¿½
		key = GetKey();
		if(sv.PHAI[0] == '\0' )
		{
...
TEN:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Ki tu: (A-Z). Toi da: 10        "); TextColor(12); gotoXY(51+vitri,y);//Ghi chï¿½
		key = GetKey();
		if(sv.HO[0] == '\0' )
		{
			GhiChu("Ban Chua Nhap HO Sinh Vien!",35,27,74,700);
...
void XuatSinhVien(ListLop &ds, ListMonHoc *root)
{
	...
	{		
		...
		for(ListSV *p = ds.l[vitriMang].Head; p != NULL; p = p->Next) 	{sv[dem1] = p->sv; dem1++;} //Dï¿½? DSSV vï¿½o mang?
...
Xuat:
	...
	while(1)
	{
		...
		{
			putchar(7);	TextColor(250);	gotoXY(47,10);printf("DELETE"); //Tï¿½ mï¿½u DELETE
		...
		if(key == ESC)
		{
			if(check_change == true)
			{
				putchar(7); TextColor(251);	gotoXY(74,10); printf("ESC"); //Tï¿½ mï¿½u ESC
				int temp = ThongBao_ESC();
				if(temp == -1)
				{
					TextColor(12);
	...
	while(1)
	{
		...
		if(key == ESC)
		{
			if(check_change == true)
			{
				...
				if(temp == -1)	//-1 la Bat duoc 1 phim ESC nua~
				{
					gotoXY(4,24);  TextColor(252);	printf("ESC"); TextColor(91); //Tï¿½ mï¿½u ESC
					gotoXY(10+vitri,y);
					continue;
				}	
				if(temp == 1)	
	...
	while(1)
	{
		...
		if(key == ESC)
		{
			if(check_change == true)
			{
				...
				if(temp == -1)	//-1 la Bat duoc 1 phim ESC nua~
				{
					gotoXY(4,24);  TextColor(252);	printf("ESC"); continue; //Tï¿½ mï¿½u ESC
				}	
				if(temp == 1)	
				{
					GhiChu("Luu Vao File Thanh Cong!",35,27,74,2000); 
...
TENMH:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Ki tu: (A-Z) & (0-9). Toi da: 35"); TextColor(12); gotoXY(30+vitri,y); //Ghi chï¿½
		key = GetKey();
		if(mh.MAMH[0] == '\0' )
		{
			GhiChu("Ban Chua Nhap MA Mon Hoc!",35,27,74,700);
...
STCLT:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Ki tu: (0-9). Toi da: 3         ");	TextColor(12); gotoXY(67+vitri,y); // Ghi chï¿½
		key = GetKey();
		if(mh.TENMH[0] == '\0' )
		{
			GhiChu("Ban Chua Nhap TEN Mon Hoc!",35,27,74,700);
...
STCTH:
	while(1)
	{
		TextColor(106);	gotoXY(10,15); printf("Ki tu: (0-9). Toi da: 3         ");	TextColor(12); gotoXY(75+vitri,y); // Ghi chï¿½
		key = GetKey();
		if(mh.STCLT[0] == '\0' )
		{
...
void Nhap_Xuat_Diem(ListLop &dsLOP, ListMonHoc *&root, int chedo)//Neu chedo = 1: Nhap Diem		//chedo = 0 : Xuat Diem
{
	...
	MonHoc mh;  mh.MAMH[0] = NULL; 	dsLOP.l[dsLOP.n].MALOP[0] = NULL; //Khoi tao rong~     //dem: dï¿½m' dï¿½ng 	dem2: dem diem?
		...
		if(key == ESC)
		{
			...
			if(err == 1)
			{
				...
				{			
					...
					{
						strcpy(temp->d.DIEM, diem[i++].DIEM);	//Neu trung` MAMH, Lanthi, thi cap nhat diem sï¿½' moi' vao`
						continue;
					}
			...
			if(err == 0)
			{
				...
				{			
					...
					if(temp != NULL) //Kiem tra xem co trung` MAMH, LAN trong listdiem ko
					{
						strcpy(temp->d.DIEM, diem[i++].DIEM);	//Neu trung` MAMH, Lan, thi cap nhat diem sï¿½' moi' vao`
						continue;
					}
					InsertTail_Diem(p->sv.Head,diem[i++]);	//Neu khong trung` them vao DSLK Don 1 Node moi'
				}
